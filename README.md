...
### DESCRIPTION:
* This is a Llama 2 language model and llama-cpp based llm engine/interface in the form of an Advanced Chatbot. It uses Python scripts, YAML files, and ASCII art to produce context-aware conversations. The scripts can be modified this then becomes a framework for forked AI Systems for your own projects, this could include, file management and content editing. Due to the way its done, even though the window_1 uses WSL/linux, the second window is using windows/linux python, and so, sounds and TTS, works in windows, which is nice! However here is the projects issue for me, it tries to maintain compatibility with both WSL/windows and Linux, hence, I will try to complete the project (see future updates section), never the less, Llama2Robot stands as a great resource for people looking to develop their own AI system, and me for one, got to the next stage of figuring it all out, which to note, I determined to be, to re-write from scratch as a, powershell and llama-cpp binaries and named pipes, version. 
...
* Large Language Models in the format of, Llama 2 GGUF, that could be between, 2GB and 40GB, or more if you should choose, and are typically sourced from website "www.huggingface.co", such as the [Llama 2 Chat 8bit GGUF](https://huggingface.co/TheBloke/Llama-2-7b-Chat-GGUF/resolve/main/llama-2-7b-chat.Q8_0.gguf), there are obvious grammatical errors on a 2 Bit, but working on code alone requires [smallest](https://huggingface.co/TheBloke/Llama-2-7b-Chat-GGUF) model for faster loading.
...
* This program is in no way affiliated with the producers of, Llama 2 or GGUF, the name means the program runs through the use of such things as, Llama2 and GGUF, however, Wiseman-Timelord may have met a few, Llamas and people with the surname Gguf, at some point also. The model was chosen as it was the only model reported to be able to correctly produce a json in reviews on youtube at the time. 
