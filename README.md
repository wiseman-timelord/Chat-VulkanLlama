...
* This program is in no way affiliated with the producers of, Llama 2 or GGUF, it merely is a Chatbot that runs through the use of such things. The naming is intended for understanding of elements involved with the configuration of the product, so that the user immediately understand if they have a grasp of the forefillment of the requirements being the, downloading and storage, of large langualge models in the format of, Llama 2 or GGUF, that could be between, 2GB and 40GB, and are typically sourced from website "www.huggingface.co".
* We are using Llama 2 because it is the only local language model that has been reviewed on YouTube to my knowing at the time of, inception and creation, of Robot, this was the GGML variant of Llama2 model, in its quantized format, and the only one that are able to correctly write a ".json" file correctly, thereby denoting compitence on multi-line output. Thus if this were the case with other models beforehand, it would be named after them.
