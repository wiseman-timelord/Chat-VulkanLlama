...
### DESCRIPTION:
* This is a Llama 2 language model and llama-cpp based llm engine/interface in the form of an Advanced Chatbot. It uses Python scripts, YAML files, and ASCII art to produce context-aware conversations. The scripts can be modified this then becomes a framework for forked AI Systems for your own projects, this could include, file management and content editing. Due to the way its done, even though the window_1 uses WSL/linux, the second window is using windows/linux python, and so, sounds and TTS, works in windows, which is nice! However here is the projects issue for me, it tries to maintain compatibility with both WSL/windows and Linux, hence, I will try to complete the project (see future updates section), but currently I am more interested in a windows only private project, that features, powershell and llama-cpp binaries and pipelines, never the less, Llama2Robot stands as a great resource for people looking to develop their own AI system, and me for one, got to the next stage of figuring it all out.
