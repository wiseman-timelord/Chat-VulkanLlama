...
* Large Language Models in the format of Llama 2, that could be from [2GB](https://huggingface.co/TheBloke/Llama-2-7b-Chat-GGUF/blob/main/llama-2-7b-chat.Q2_K.gguf) to [48GB](https://huggingface.co/TheBloke/Llama-2-70B-chat-GGUF/blob/main/llama-2-70b-chat.Q5_K_M.gguf), however the 2bit versions should be used for working on the code for faster loading. Currently models be in the `[INST] <<SYS>>\n{system_input}\n<</SYS>>\n{instruct_input}[/INST]` syntax only, this is planned to have a menu in future release, but for now is easily modifyable if you search for it in `./scripts/message.py`. 
